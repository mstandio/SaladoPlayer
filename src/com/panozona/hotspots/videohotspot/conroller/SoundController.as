...
package com.panozona.hotspots.videohotspot.conroller{
	...
	import com.panozona.hotspots.videohotspot.events.SoundEvent;
	import com.panozona.hotspots.videohotspot.events.StreamEvent;
	import com.panozona.hotspots.videohotspot.view.SoundView;
	...
	public class SoundController {
		...
		public function SoundController(soundView:SoundView){
			...
			_soundView.videoHotspotData.soundData.addEventListener(SoundEvent.CHANGED_MUTE, handleMuteChange, false, 0, true);
			_soundView.videoHotspotData.soundData.addEventListener(SoundEvent.CHANGED_VOLUME_BAR_OPEN, handleVolumeBarOpenChange, false, 0, true);
			_soundView.videoHotspotData.soundData.addEventListener(SoundEvent.CHANGED_VOLUME_POINTER_DRAGGED, handleVolumePointerDraggedChange, false, 0, true);
			
			_soundView.videoHotspotData.streamData.addEventListener(StreamEvent.CHANGED_VOLUME_VALUE, handleVolumeValueChange, false, 0, true);
			
			_soundView.videoHotspotData.playerData.addEventListener(PlayerEvent.CHANGED_NAVIGATION_ACTIVE, handleNavigationActiveChange, false, 0, true);
		}
		...
		private function handleMuteChange(e:Event):void {
			...
			if (_soundView.videoHotspotData.soundData.mute) {
				_soundView.soundButton.addChild(_soundView.soundOffBitmap);
				volCache = _soundView.videoHotspotData.streamData.volumeValue;
				_soundView.videoHotspotData.streamData.volumeValue = 0;
			}else {
				_soundView.soundButton.addChild(_soundView.soundOnBitmap);
				_soundView.videoHotspotData.streamData.volumeValue = (volCache == 0 ? 0.5 : volCache);
			}
		}
		
		private function handleVolumeBarOpenChange(e:Event):void {
			if (_soundView.videoHotspotData.soundData.volumeBarOpen) {
				hidingTimer.stop();
		...
		private function handleVolumeValueChange(e:Event):void {
			if (_soundView.videoHotspotData.streamData.volumeValue == 0) {
				_soundView.videoHotspotData.soundData.mute = true;
			}else {
				_soundView.videoHotspotData.soundData.mute = false;
			}
			if (!_soundView.videoHotspotData.soundData.volumePointerDragged){
		...
		private function handleNavigationActiveChange(e:Event):void {
			if (!_soundView.videoHotspotData.playerData.navigationActive) {
				hidingTimer.stop();
				_soundView.volumeBar.visible = false;
			}
		}
		...
		private function translatePositionToValue():void {
			_soundView.videoHotspotData.streamData.volumeValue = 1 -
				(_soundView.volumePointer.y - _soundView.volumePointer.height) /
				(_soundView.volumeBar.height - _soundView.volumePointer.height * 3);
		}
		
		private function translateValueToPosition():void {
			_soundView.volumePointer.y = -(_soundView.videoHotspotData.streamData.volumeValue - 1) *
					(_soundView.volumeBar.height - _soundView.volumePointer.height * 3) +
